...
class ModelCheckpoint(Callback):
  ...
  def reset(self):
    ...
    self._compare_fn = operator.gt if self.mode == 'max' else operator.lt
  ...
  def on_epoch_end(self, epoch, logs=None):
    ...
    if self.save_top_k:
      ...
      if len(self._top_k) < self.save_top_k:
        self._top_k_.append((score, path))
        self.save_checkpoint(path, epoch, logs)
      else:
        self._top_k = list(sorted(
          self._top_k,
          key=lambda x: x[0],
  ...
  def _write_logs(self, logs, index):
    for name, value in logs.items():
      ...
      with self.writer.as_default():
        ...
        tf.summary.scalar(
          name=name,
          data=(value.item if isinstance(value, np.ndarray) else value),
          step=index
        )
...
class WandB(Callback):
  def __init__(self, log_dir, config):
    ...
    path = list(p for p in log_dir.split('/') if len(p.strip()) > 0)

    import wandb
    self.wandb = wandb.init(
      project='-'.join(path[1:-1]),
      name=path[-1],
      config=config,
      reinit=True
    )
  ...
  def _write_logs(self, logs, index):
    ...
    for name, value in logs.items():
      ...
      logs_filtered[name] = value.item if isinstance(value, np.ndarray) else value

    if len(logs_filtered.keys()) > 0:
      logs_filtered['step'] = index
      self.wandb.log(logs_filtered)
...
class Scheduler(Callback):
  ...
  def on_epoch_begin(self, epoch, logs=None):
    if self.verbose:
      ...
      print('Learning rates:', '/'.join([f'{lr:6f}' for lr in lrs]))
  ...
  def on_epoch_end(self, epoch, logs=None):
    if self.freq == 'epoch':
      self.scheduler.step()
    if logs is not None:
      logs['mean_lr'] = np.mean(self.scheduler.get_lr())
...
class MetricCallback(Callback):
  ...
  def on_epoch_end(self, epoch, logs=None):
    ...
    print(f'{self.metric_name}: {score:.4f}')
...
class CacheCallback(Callback):
  def __init__(self, cache):
    super(CacheCallback, self).__init__()
    self.cache = cache
